...
function spawnSession(cmd: SpawnCmd): void {
  ...
  try {
    ...
    console.log(`[pty-daemon] Session spawned: ${id} (shell=${shell}, cwd=${cwd})`);
  } catch (err: any) {
    broadcast({ evt: 'error', id: cmd.id, error: err?.message ?? 'Failed to spawn' });
    console.error(`[pty-daemon] Spawn failed: ${cmd.id}`, err?.message);
  }
}
...
function writeToSession(id: string, data: string): void {
  ...
  if (session) {
    try {
      ...
      console.error(`[pty-daemon] Write failed: ${id}`, err?.message);
    }
  }
}
...
function resizeSession(id: string, cols: number, rows: number): void {
  ...
  if (session) {
    try {
      ...
      console.error(`[pty-daemon] Resize failed: ${id}`, err?.message);
    }
  }
}
...
function killSession(id: string): void {
  ...
  if (session) {
    ...
    console.log(`[pty-daemon] Session killed: ${id}`);
    resetIdleTimer();
  }
}
...
function captureSession(id: string): string | null {
  ...
  try {
    ...
    console.error(`[pty-daemon] Capture failed: ${id}`, err?.message);
    return null;
  }
}
...
function resetIdleTimer(): void {
  ...
  if (sessions.size === 0 && clients.size === 0) {
    idleTimer = setTimeout(() => {
      console.log('[pty-daemon] Idle timeout — shutting down');
      gracefulShutdown();
    }, IDLE_TIMEOUT_MS);
  }
}
...
const server = Bun.listen({
  ...
  socket: {
    ...
    data(socket, data) {
      ...
      while ((newlineIdx = buffer.indexOf('\n')) !== -1) {
        ...
        try {
          ...
          console.error(`[pty-daemon] Invalid message: ${line.slice(0, 200)}`);
        }
      }
    ...
    error(_socket, error) {
      console.error('[pty-daemon] Socket error:', error.message);
    },
  },
});
...
console.log(`[pty-daemon] Started (pid=${process.pid})`);
console.log(`[pty-daemon] Socket: ${SOCKET_PATH}`);
console.log(`[pty-daemon] PID file: ${PID_FILE}`);
